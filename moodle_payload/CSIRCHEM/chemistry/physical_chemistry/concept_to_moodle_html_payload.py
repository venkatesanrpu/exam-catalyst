...
def generate_html_for_concept(concept, metadata, lesson_topic, subject_snake):
    ...
    # Combine all links
    all_links = f"{clarifier} | {study_notes} | {mcq_basic} | {mcq_intermediate} | {mcq_advanced}"

    return f"<li>{all_links}</li><br>"
...
def process_yaml_file(yaml_path, csv_mapping, output_folder):
    ...
    filename_prefix = filename.replace('_concepts.yaml', '')
...
def main():
    ...
    yaml_files = list(Path(folder_path).glob('*_concepts.yaml'))

    if not yaml_files:
        print(f"No *_concepts.yaml files found in '{folder_path}'")
        return
